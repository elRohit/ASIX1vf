...
DO $$ 
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'detall_tiquet') THEN
        DROP TABLE detall_tiquet;
    END IF;
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'tiquet') THEN
        DROP TABLE tiquet;
    END IF;
    ...
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'caixa') THEN
        DROP TABLE caixa;
    END IF;
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'stock_prod') THEN
        DROP TABLE stock_prod;
    END IF;
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'prod_prov') THEN
        DROP TABLE prod_prov;
    END IF;
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'proveidor') THEN
        DROP TABLE proveidor;
    END IF;
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'producte') THEN
        DROP TABLE producte;
    END IF;
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'empleat') THEN
        DROP TABLE empleat;
    END IF;
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'supermercat') THEN
        DROP TABLE supermercat;
    END IF;
END $$;*/
/*
*********************************************************************************************************************
...
VALUES ('11111111A', 'Ricard', 'Pérez', 'Osorio', 2),
       ...
       ('00000000J', 'Adrian', 'Gimenez', 'Romero', 1);
...
VALUES ('A12345678', 'Heineken', 'Carrer SuperAlcoholica Embrutada 3', 'sells@heineken.com', '123456789', 'Ariadna'),
       ...
       ('C11111111', 'Florichsol', 'Carrer Flor 3', 'proveidorC@example.com', '111111111', 'Pedro'),
       ('D99999999', 'StarLiquors', 'Carrer Liquor nº 1', 'proveidorD@example.com', '999999999', 'Rocky');
